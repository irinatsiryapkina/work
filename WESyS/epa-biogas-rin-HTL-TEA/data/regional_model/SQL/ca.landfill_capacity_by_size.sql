CREATE OR REPLACE VIEW "ca"."landfill_capacity_by_size" AS (
...
COMMENT ON VIEW "ca"."landfill_capacity_by_size" IS 'Computes average landfill capacity for CA from LMOP data by size (small vs. large) and status (open (''active'') vs. closed (''inactive'')). Corresponds to data for model variables: Avg_landfill_capacity_by_size and INIT_per_LF_loading.'
;
