...
get.ad <- function (selection, bbar, design, boots, design.melted = FALSE, pvalue){
  options (digits = 10)
  B <- selection
  ...
  #pvalues are based on bootstraps
        ...
        for (i in 1:length (B.cdf)){
          LIST [[i]] <- ad.test (B.cdf[[i]], B.bar.cdf[[i]], method = "exact", Nsim = boots) #Nsim = bootstraps
          ad.list <- (LIST)  
        }
  ...
  setnames(ad.results, c("factor", "p.value")) 
  ...
  sig.results$p.value <- -log10(sig.results$p.value)
  
  sig.results <- subset (ad.results, ad.results$p.value < pvalue) #0.000000001)#subset the AD results based on p-value
  ...
  results <- data.table (factor = sig.results$factor, p.value = sig.results$p.value, 
                         B.med = B.summary$Mean, B.bar.med = B.bar.summary$Mean)
  return (results)
