run.mood <- function (selection, bbar, design, boots, design.melted = FALSE, alpha){
  ...
  B <- sig.results.design
  B.bar$type <- 'B.bar'
  B$type <- 'B'
  print(head(B))
  print(head(B.bar))
  combined_output <- rbind(B, B.bar)
  #return (B.bar.summary, B.summary, B.bar, B)
