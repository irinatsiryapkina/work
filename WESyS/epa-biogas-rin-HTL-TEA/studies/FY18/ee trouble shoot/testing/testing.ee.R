...
foo <- data.table (f1 = runif(n = 1000, min = 1, max = 6000), f2 = rnorm (n = 1000, sd = 50, mean = 125), 
                   f3 = seq (from = 3001, to = 4000, by = 1), f4 = seq (from = 0.001, to = 1, by = 0.001),
                   f5 = rpois (1000, 5))
...
vars <- read.csv ("foo.config.csv", header = TRUE, fileEncoding = "UTF-8-BOM")
...
foo.output <- predict.lm (foo.model, new.design, interval = "prediction", type = "response")

foo.output <- melt (foo.output [,1])

ee.results <- ee (SA.design, foo.output$value, N, r)

#ee.results <- join (vars, ee.results, by = "set")

ee.results <- arrange (ee.results, desc(std.errors))

plot (ee.results$mu.star, ee.results$sigma)

#now try with rescaled factors
scale01 <- function(x){(x-min(x))/(max(x)-min(x))} #rescales a vector to (0,1)
#scaled.design <- apply (new.design, 2, scale)
scaled.output <- scale01 (foo.output$value)

scaled.ee <- ee (SA.design, scaled.output, N, r)


##########new function here##########
ee.scaled <- function(x = NULL, y = NULL, N = NULL, r = NULL) {
  m <- (N + 1) * r
  
  #scale the design
  x <- dcast (x, run ~ variable, value.var = "value")
  #run <- x$run
  x <- x[,-c(1)]
  x <- apply (x, 2, scale)
  x <- melt (t(x))
  #x <- cbind (run, x[,2:3])
  colnames (x) <- c ("variable", "run", "value")
  ...
  #xdiff <- (x[2:m, ] - x[1:(m-1), ])[-(N+1)*1:(r-1), ]
  x1 <- x[,3]
  xdiff <- (x1[2:m] - x1[1:(m-1)])[-(N+1)*1:(r-1)]
  ...
  index <- rep(0:(r-1)*(N + 1), each=N) 
  
  test <- apply(xdiff, 2, function(x) (1:N)[x != 0])
...
scaled.ee.results <- ee (ee.design, foo.output$value, N, r)
